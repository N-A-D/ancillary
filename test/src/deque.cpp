...
using deque_t = ancillary::deque<int>;
using deque2_t = ancillary::deque<int, my_allocator<int>>;
...
TEST(DequeTests, CapacityTests) {
	...
	deque.shrink_to_fit();
	auto invariant(deque);
	ASSERT_EQ(deque.capacity(), deque.size());
	ASSERT_EQ(invariant, deque);
	deque.reserve(deque.capacity() * 2);
	ASSERT_NE(deque.capacity(), deque.size());
	ASSERT_EQ(invariant, deque);
	deque.clear();
...
TEST(DequeTests, SwapTests) {
	ancillary::deque<int> d1 = { 1, 2, 3, 4 };
	ancillary::deque<int> d2 = { 5, 6, 7, 8 };

	std::swap(d1, d2);

	ASSERT_EQ(d1, ancillary::deque<int>({ 5, 6, 7, 8 }));
	ASSERT_EQ(d2, ancillary::deque<int>({ 1, 2, 3, 4 }));

	ancillary::deque<int, my_allocator<int>> d3 = { 1, 2, 3, 4 };
	ancillary::deque<int, my_allocator<int>> d4 = { 5, 6, 7, 8 };

	std::swap(d3, d4);

	// No change since my_allocator does not propagate on swap or is always equal

	ancillary::deque<int, my_allocator<int>> y{ 1, 2, 3, 4 };
	ancillary::deque<int, my_allocator<int>> z{ 5, 6, 7, 8 };
